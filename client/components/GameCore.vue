...
export default V.extend({
  ...
  methods: {
    ...
    collisionDetection() {
      ...
      if (
        this.ball.x + this.ball.radius >= this.position.x &&
        this.ball.x - this.ball.radius <= this.position.x + 20 &&
        this.ball.y + this.ball.radius >= this.position.y &&
        this.ball.y - this.ball.radius <= this.position.y + 120
      ) {
        this.ball.x += 4
        this.direction.x = -this.direction.x
      } else if (
        this.ball.x + this.ball.radius >= this.position2.x &&
        this.ball.x - this.ball.radius <= this.position2.x + 20 &&
        this.ball.y + this.ball.radius >= this.position2.y &&
        this.ball.y - this.ball.radius <= this.position2.y + 120
      ) {
        this.ball.x -= 4
        this.direction.x = -this.direction.x
      }
    ...
    updateBall() {
      ...
      if (
        this.ball.x >= this.position.x &&
        this.ball.x <= this.position.x + 20 &&
        this.ball.y >= this.position.y &&
        this.ball.y <= this.position.y + 120
      ) {
        this.direction.x = -this.direction.x
      }
      // on verifie si la balle touche le joueur 2
      if (
        this.ball.x >= this.position2.x &&
        this.ball.x <= this.position2.x + 20 &&
        this.ball.y >= this.position2.y &&
        this.ball.y <= this.position2.y + 120
      ) {
        this.direction.x = -this.direction.x
      }
      if (this.velocity.speed < 0.05) this.velocity.speed += 0.00005
    },
    move(direction: string) {
      if (this.score.player1 >= 5 || this.score.player2 >= 5) return
