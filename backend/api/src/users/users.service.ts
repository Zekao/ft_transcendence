...
export class UsersService {
  ...
  async getFriends(id:string): Promise<UserDto[]> {
	...
	const friends : UserDto[] = user.friends.map((friend) => {return new UserDto(friend);});
	return friends;
  }
  ...
  async getUserId(id: string, RelationsPicker ?: UserRelationsPicker): Promise<User> {
	...
	let found = null;
	if (isUuid(id))
	  found = await this.UserRepository.findOne({ where: { id: id }, relations });
	else
	  found = await this.UserRepository.findOne({ where: { user_name: id }, relations });
	if (!found)
  ...
  async addFriend(id : string, friend_id: string) : Promise<User> {
	if (friend_id == id) throw new BadRequestException("You can't add yourself");
