...
let ChannelsService = class ChannelsService {
    ...
    async getPrivateChannel(user) {
        ...
        privateChannel = privateChannel.filter((channel) => channel.members.find((member) => member.id === user.id) ||
    ...
    async deleteChannelMember(me, channelId, channelMembers) {
        const { user } = channelMembers;
        if (user)
            me = user;
        const found = await this.UsersService.getUserId(me);
    ...
    async deleteChannelAdmin(me, channelId, channelMembers) {
        const { user } = channelMembers;
        if (user)
            me = user;
        const found = await this.UsersService.getUserId(me);
    ...
    async deleteChannelMute(me, channelId, channelMembers) {
        const { user } = channelMembers;
        if (user)
            me = user;
        const found = await this.UsersService.getUserId(me);
    ...
    async deleteChannelBan(me, channelId, channelMembers) {
        const { user } = channelMembers;
        if (user)
            me = user;
        const found = await this.UsersService.getUserId(me);
